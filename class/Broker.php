...
class Broker
{
    ...
    // PRIVATE METHOD TO UPLOAD FILE AND RETURN THE NAME OF THE FILE IF SUCCESSFUL
    protected function uploadDocument(string $file): string|bool
    {
        try {
            $time = time();
            $file_name = $time . $_FILES[$file];
            $file_tmp_name = $_FILES['tmp_name'];
            $file_destination_path = '../../assets/image' . $file_name;


            $allowed_files = ['pdf', 'csv', 'doc'];
            $tmpextention = explode('.', $file_name);
            $extention = strtolower(end($tmpextention));

            if (in_array($extention, $allowed_files)) {

                if ($_FILES['size'] < 5_000_000) {
                    move_uploaded_file($file_tmp_name, $file_destination_path);
                }
            }
            
        } catch (Exception $e) {
            throw new BrokerExpection("cannot upload: " . $e->getMessage());
        }
        return $file_name;
    }
     ...
     */
    public function createDocument(string $name, string $type, int $priority): int
    {
        try {
            $cols = [
                tblDocument::NAME => [$name, 'isValue'], tblDocument::TYPE => [$type, 'isValue'], tblDocument::PRIORITY => [$priority, 'isValue']
            ];
